...
Representa, d’una banda, com funciona un sistema d’economia lineal existent a la teva zona i, de l’altra, un model d’economia circular. Intenta que els punts, llocs i processos que incorporis al mapa siguin el més pròxims i reals possibles a la teva realitat quotidiana.
...
2. **Configura capes al mapa**. Crea una capa per cada tipus d'economia:
   ...
   → **Producció** : Es fabrica el telèfon mòbil amb un sistema desmuntable.

   → **Consum** : El consumidor el compra i el fa servir fins que s'espatlla.

   → **Recollida** : El consumidor el lliura a un punt verd de deixalleria.

   → **Reciclatge** : Es desmunta el mòbil recuperant el liti i el coltan per fabricar la següent generació de telèfons. No tornem a la mina, tornem a la producció.
...
3. **Afegeix punts a cada capa**. A cada capa hauràs d'afegir-hi punts per representar cada una de les fases del model.
   ...
   - El **nom** de cada punt ha d'indicar a quina fase pertany.
   - Assigna **icones representatives** a cada punt.
   - Inclou una **descripció breu i una imatge representativa** a cada punt.
   - Opcionalment, pots fer que es mostri el nom dels punts directament al mapa.

5. **Traça línies de flux** per indicar la seqüència del recorregut entre punts.  

6. **Mesura la distància** de cada recorregut dissenyat i escriu-la dins la descripció de la línia.  
...
- Calcula i compara les distàncies totals de transport de cada model i analitza com aquestes distàncies poden influir en la petjada de carboni.
